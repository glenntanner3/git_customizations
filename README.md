...
## The Basics
#### Cloning (Downloading) the repository to your system:
* Windows (cmd or powershell): `git clone git@gitlab.amzetta.com:testing-automation/automation---sikulix-.git %AppData%\Sikulix`
* Linux: `git clone git@gitlab.amzetta.com:testing-automation/automation---sikulix-.git ~/Sikulix`
...
#### Merge request
1. In GitLab web UI view Repository [Branches](https://gitlab.amzetta.com/testing-automation/automation---sikulix-/branches)
1. Click 'Merge Request' on your branch
1. Do not select the 'delete branch' or 'squash' options
1. Select the desired template
1. Developer - Complete developer merge request checklist
1. Assignee - Complete assignee merge request checklist
1. After merge, delete the source branch. There is a button on the MR to delete the source branch.
...
* **lsb** - List all existing branches. It preforms a fetch then list's all remote branches.
...
* **sync** - Sync branch with origin/dev - Checks that there is nothing needing to be committed, fetches origin, merges orgin/dev into the current branch, if successful pushes the merge.
   * `git config --global alias.sync '!git diff-index --quiet HEAD -- && (git fetch -p;git merge origin/dev && git push || echo "Not pushed due to merge conflict") || echo "Aborted: Uncommitted changes"'`
#### Stashing
